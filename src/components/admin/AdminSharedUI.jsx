...
export const FLabel = ({ label, children, span2 }) => (
  ...
  >
    <label className="text-[10px] font-bold text-gray-400 tracking-[1.2px] font-['Outfit',sans-serif] uppercase">
      {label}
    </label>
    {children}
  </div>
...
export const ModalHeader = ({ title, sub, onClose }) => (
  <div className="px-7 pt-6 pb-5 border-b border-slate-100 flex justify-between items-start">
    <div>
      <div className="text-[10px] text-blue-500 font-bold tracking-[1.5px] font-['Outfit',sans-serif] mb-1 uppercase">
        {sub}
      </div>
      <h2 className="font-['Playfair_Display',serif] text-[21px] font-extrabold text-[#0c0e1a]">
        {title}
      </h2>
    </div>
    <button
...
export const DeleteModal = ({ label, onConfirm, onClose, isLoading }) => (
  <Overlay onClose={onClose}>
    <div className="px-8 py-10 text-center">
      ...
      <h2 className="font-['Playfair_Display',serif] text-[19px] text-[#0c0e1a] mb-2.5 font-bold">
        Confirm Delete
      </h2>
      <p className="text-gray-500 text-sm mb-7 font-['Outfit',sans-serif] leading-relaxed">
      ...
      <div className="flex gap-2.5 justify-center">
        <button
          onClick={onConfirm}
          disabled={isLoading}
          className={`px-7 py-2.5 rounded-xl border-none font-['Outfit',sans-serif] text-sm font-bold transition-all ${isLoading ? "bg-red-200 text-white cursor-not-allowed opacity-60" : "bg-red-600 hover:bg-red-700 text-white cursor-pointer shadow-lg shadow-red-600/20"}`}
        >
        ...
        <button
          onClick={onClose}
          disabled={isLoading}
          className={`px-7 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-gray-400 font-['Outfit',sans-serif] text-sm font-semibold transition-all ${isLoading ? "cursor-not-allowed" : "hover:bg-slate-100 hover:text-gray-600 cursor-pointer"}`}
        >
          Cancel
        </button>
      </div>
