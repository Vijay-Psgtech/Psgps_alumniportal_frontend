  ...
  return (
    <div className="relative">
      ...
      {isFetching && (
        ...
        >
          <div className="text-center">
            ...
            />
            <p className="text-gray-600 font-['Outfit',sans-serif] text-sm font-medium">
              Loading events...
            </p>
          </div>
        </motion.div>
      ...
      <div className="flex flex-wrap items-center gap-2.5 mb-4 border-b border-transparent pb-1">
        <div className="flex-1 min-w-[200px] relative">
          ...
          <input
            ...
            className="w-full py-2.5 pr-3.5 pl-9 border border-slate-200 rounded-xl font-['Outfit',sans-serif] text-[13px] outline-none bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all"
          />
        </div>
        <div className="flex bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm shadow-black/5">
          {["all", "upcoming", "completed"].map((f) => (
            <button
              ...
              className={`px-4 py-2 border-none font-['Outfit',sans-serif] text-xs font-bold transition-colors capitalize ${statusFilter === f ? "bg-[#667eea] text-white" : "bg-transparent text-gray-400 hover:bg-slate-50 hover:text-gray-600"}`}
            >
              {f}
            </button>
          ))}
        ...
        <button
          ...
          className="px-4 py-2.5 rounded-xl border-none bg-linear-to-br from-[#667eea] to-[#764ba2] text-white font-['Outfit',sans-serif] text-[13px] font-bold cursor-pointer flex items-center gap-1.5 whitespace-nowrap shadow-md shadow-blue-500/25 transition-all hover:-translate-y-0.5 hover:shadow-blue-500/40"
        >
          <Plus size={14} /> Add Event
        </button>
      </div>

      <div className="text-xs text-gray-400 font-['Outfit',sans-serif] mb-3 font-medium">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
        <div className="hidden sm:grid grid-cols-[2.4fr_1fr_1fr_1fr_0.6fr_90px] px-4 py-3 bg-slate-50 border-b border-slate-200">
          {["Title", "Category", "Date", "Status", "Attend.", "Actions"].map(
            (h) => (
              <span
                key={h}
                className="text-[12px] font-bold text-gray-700 tracking-wide font-['Outfit',sans-serif] uppercase"
              >
                {h}
              </span>
            ),
        ...
        {filtered.length === 0 && (
          <div className="p-12 text-center text-gray-400 font-['Outfit',sans-serif]">
            ...
            return (
              ...
              >
                <div>
                  <div className="flex items-center gap-3 mb-0.5">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold text-[16px] text-[#0c0e1a] font-['Outfit',sans-serif] leading-tight">
                        {ev.title}
                      </span>
                    </div>
                  </div>
                  <span className="text-[12px] text-gray-500 font-['Outfit',sans-serif] block line-clamp-1">
                    {ev.venue?.split(",")[0]}
                  </span>
                </div>
                ...
                <div className="hidden sm:block">
                  <span
                    className="inline-block rounded-full px-2.5 py-1 text-[13px] font-bold font-['Outfit',sans-serif] tracking-wide"
                    style={{
                      background: `${cc}15`,
                      color: cc,
                ...
                <span className="text-sm text-gray-500 font-['Outfit',sans-serif] font-medium hidden sm:block">
                  {fmtDate(ev.date)}
                </span>
                <span
        ...
        {totalPages > 1 && (
          <div className="flex justify-end items-center gap-2 px-4 py-3 bg-slate-50 border-t border-slate-200">
            <button
              ...
              className={`px-3 py-1 rounded-md border border-slate-300 bg-white text-sm font-['Outfit',sans-serif] ${currentPage === 1 ? "cursor-not-allowed opacity-50" : "hover:bg-slate-100"}`}
            >
              Previous
            </button>
            <span className="text-sm text-gray-500 font-['Outfit',sans-serif]">
              Page {currentPage} of {totalPages}
            </span>
            <button
              onClick={() => setCurrentPage((p) => Math.min(p + 1, totalPages))}
              disabled={currentPage === totalPages}
              className={`px-3 py-1 rounded-md border border-slate-300 bg-white text-sm font-['Outfit',sans-serif] ${currentPage === totalPages ? "cursor-not-allowed opacity-50" : "hover:bg-slate-100"}`}
            >
              Next
            </button>
          </div>
